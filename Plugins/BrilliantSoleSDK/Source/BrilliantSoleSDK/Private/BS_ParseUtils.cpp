...
void UBS_ParseUtils::ParseServerData(const TArrayView<const uint8> &Data, FBS_ServerMessageCallback MessageCallback)
{
...
void UBS_ParseUtils::ParseDeviceEventData(UBS_Device *Device, const TArray<uint8> &Data, FBS_DeviceEventCallback MessageCallback)
{
    ...
    while (Offset < DataLength)
    {
        ...
        const TArrayView<const uint8> MessageData((uint8 *)(Data.GetData() + Offset), MessageDataLength);
        MessageCallback.ExecuteIfBound(Device, MessageType, MessageData);
